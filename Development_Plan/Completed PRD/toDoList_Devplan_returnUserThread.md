...
- [x] Add `return_user_probe` node; wire: `ingest -> return_user_probe -> profile_builder`.
...
- [x] Implement `return_user_probe` to load persisted snapshots (company + ICP) from Postgres by tenant.
  ...
  - Set: `state.is_return_user = True` when snapshots exist; compute `state.decisions` (use_cached vs rerun_icp) by diffs/staleness and LLM policy (fallback to heuristics).
  - Accept: On an existing tenant with snapshots, chat skips redundant asks.

## 3) journey_guard — Queue Background Job Only
- [x] Always queue unified background job when `company_profile_confirmed` and `icp_profile_confirmed` are true; no inline discovery/enrich.
...
- [x] Single active thread per context; compute and store `context_key`; auto‑lock prior open threads.
...
- [x] Durable checkpointer for threads/runs in embedded server.
...
- [x] Chat “status” intent (optional): summarize background job progress on demand.
