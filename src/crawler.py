    ...
    async with httpx.AsyncClient() as client:
        ...
        for u in allowed:
            try:
                ...
                page_url, page_html = await _fetch(client, u)
                s2 = _extract_signals(page_html, base)
                pages.append(
                    {"url": page_url, "html": page_html, "title": s2.get("title") or ""}
                )
                for k in ["value_props", "products_services", "pricing", "languages"]:
                    signals[k] = sorted(
                        set((signals.get(k) or []) + (s2.get(k) or []))
                    )[:40]
                c = signals.get("contact", {"emails": [], "phones": []})
                c2 = s2.get("contact", {"emails": [], "phones": []})
                c["emails"] = sorted(set(c.get("emails", []) + c2.get("emails", [])))[
                    :40
                ]
                c["phones"] = sorted(set(c.get("phones", []) + c2.get("phones", [])))[
                    :40
                ]
                signals["contact"] = c
                for bucket, arr in (s2.get("tech") or {}).items():
                    signals["tech"].setdefault(bucket, [])
                    signals["tech"][bucket] = sorted(
                        set(signals["tech"][bucket] + arr)
                    )[:40]
