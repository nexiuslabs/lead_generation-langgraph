...
async def _deterministic_crawl_and_persist(
    company_id: int, url: str, persist_pages: bool = False
):
    ...
    # Store in summaries table and optionally persist per-page HTML
        ...
        if persist_pages:
            try:
                ...
                for p in pages[:CRAWLER_MAX_PAGES]:
                    cur.execute(
                        """
                        INSERT INTO crawl_pages (company_id, url, title, raw_html, fetched_at)
                        VALUES (%s, %s, %s, %s, now())
        ...
        try:
            if state.get("company_id") and state.get("home"):
                await _deterministic_crawl_and_persist(
                    state["company_id"], state["home"], persist_pages=True
                )
