...
async def _deterministic_crawl_and_persist(company_id: int, url: str) -> bool:
    """Run deterministic crawler, persist results and return True if content was captured."""
    try:
    ...
    return bool(summary.get("content_summary"))
    ...
    if not extracted_pages:
        try:
            if state.get("company_id") and state.get("home"):
                _ = await _deterministic_crawl_and_persist(
...
async def node_deterministic_fallback(state: EnrichmentState) -> EnrichmentState:
    if state.get("completed") or not state.get("home"):
        return state
    try:
        success = await _deterministic_crawl_and_persist(
            state["company_id"], state["home"]
        )
        if success:
            state["completed"] = True
...
enrichment_graph.add_edge("find_domain", "deterministic_fallback")
enrichment_graph.add_edge("deterministic_fallback", "discover_urls")
