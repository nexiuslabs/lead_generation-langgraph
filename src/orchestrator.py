...
from src.settings import ICP_RULE_NAME, POSTGRES_DSN
...
def fetch_industry_codes_by_names(industries):
    ...
    Look up matching codes in ``ssic_ref`` by description/title, falling
    back to existing ``companies`` entries if needed. Returns a sorted list
    ...
    conn = psycopg2.connect(dsn=POSTGRES_DSN)
    try:
        # Primary lookup via ssic_ref table
        with conn, conn.cursor() as cur:
            cur.execute(
                """
                SELECT DISTINCT CAST(ssic_code AS TEXT)
                FROM public.ssic_ref
            ...
            for (code,) in rows:
                if code:
                    codes.add(str(code))
        # Fallback via companies table if ssic_ref yields nothing
        if not codes:
            with conn, conn.cursor() as cur:
                cur.execute(
                    """
...
def output_candidate_records(company_ids):
    ...
    with conn, conn.cursor() as cur:
        cur.execute(
            "SELECT * FROM companies WHERE company_id = ANY(%s)", (company_ids,)
        )
...
async def main():
    ...
    logging.info(
        f" ✅ Matched ICP candidate IDs: {icp_result_state['candidate_ids']} (count={len(icp_result_state['candidate_ids'])})"
    )
    ...
    if not candidate_ids and icp_payload.get("industries"):
        industries_norm = sorted(
            {
                (s or "").strip().lower()
                for s in icp_payload["industries"]
                if isinstance(s, str) and s.strip()
            }
        )
        ...
        if codes:
            ...
            fallback_ids = []
            logging.info(
                "No industry codes derived; no fallback candidates found via codes"
            )
    ...
    logging.info(
        f"Normalized to {len(norm_result_state['normalized_records'])} companies"
    )
